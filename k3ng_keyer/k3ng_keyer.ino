...
#include <EEPROM.h>
#include <ESP32Encoder.h>  //BG4FQD Encoder https://github.com/madhephaestus/ESP32Encoder/
#else
#include <avr/pgmspace.h>
#include <avr/wdt.h>
#include <EEPROM.h>
...
// Variables and stuff
struct config_t {  // 120 + 54 + 54bytes total
  uint8_t paddle_mode;
  uint8_t keyer_mode;
  ...
  char APSSID[33];
  char APPassword[17];
  char TelnetServerEnable;
  char WebServerEnable;
  // 104 bytes
} configuration;


#if defined(ARDUINO_ARCH_ESP32) && defined(FEATURE_ROTARY_ENCODER)
...
void loop() {
  ...
  if (runTinyFSK) {
    ...
    if (keyer_machine_mode == KEYER_NORMAL) {
      ...
      check_buttons();
#endif
      //check_paddles();  //BG4FQD
      //service_dit_dah_buffers();  //BG4FQD
...
long service_straight_key() {
  ...
  if (digitalRead(pin_straight_key) == STRAIGHT_KEY_ACTIVE_STATE) {
    ...
    if (!last_straight_key_state) {
      //#if defined(DEBUG_FEATURE_STRAIGHT_KEY_ECHO)
      //  debug_serial_port->println("Straight key Down. ");
      //#endif
  ...
  while ((keyboard.available()) && (play_memory_prempt == 0)) {
    ...
    if (ps2_keyboard_mode == PS2_KEYBOARD_NORMAL) {
      switch (keystroke) {
        ...
        case PS2_F1_ALT:
          if (number_of_memories > 0) { repeat_memory_msg(0); }
          break;
        case PS2_F2_ALT:
          if (number_of_memories > 1) { repeat_memory_msg(1); }
          break;
        case PS2_F3_ALT:
          if (number_of_memories > 2) { repeat_memory_msg(2); }
          break;
#ifndef OPTION_SAVE_MEMORY_NANOKEYER
        case PS2_F4_ALT:
          if (number_of_memories > 3) { repeat_memory_msg(3); }
          break;
        case PS2_F5_ALT:
          if (number_of_memories > 4) { repeat_memory_msg(4); }
          break;
        case PS2_F6_ALT:
          if (number_of_memories > 5) { repeat_memory_msg(5); }
          break;
        case PS2_F7_ALT:
          if (number_of_memories > 6) { repeat_memory_msg(6); }
          break;
        case PS2_F8_ALT:
          if (number_of_memories > 7) { repeat_memory_msg(7); }
          break;
        case PS2_F9_ALT:
          if (number_of_memories > 8) { repeat_memory_msg(8); }
          break;
        case PS2_F10_ALT:
          if (number_of_memories > 9) { repeat_memory_msg(9); }
          break;
        case PS2_F11_ALT:
          if (number_of_memories > 10) { repeat_memory_msg(10); }
          break;
        case PS2_F12_ALT:
          if (number_of_memories > 11) { repeat_memory_msg(11); }
          break;
#endif  //OPTION_SAVE_MEMORY_NANOKEYER
#endif  //FEATURE_MEMORIES

        case PS2_DELETE:
          if (send_buffer_bytes) { send_buffer_bytes--; }
          break;
...
int chk_rotary_encoder() {
  static unsigned long LastStep = INT_MAX;
  int result = 0;
  long step = encoder.getCount();
  if (LastStep == INT_MAX)
    LastStep = step;
    ...
    if ((LastStep > step))
      ...
      result = 1;  //cw
    LastStep = step;
#if defined(DEBUG_CW_DECODER)
    debug_serial_port->print(" Encoder step ");
    debug_serial_port->print(step);
    debug_serial_port->print(" Direct ");
    debug_serial_port->println(result);
#endif
  }
  return result;
}
...
#endif    // End of ARDUINO_ARCH_ESP32
...
void check_sidetone_switch() {
  ...
  if ((ss_read == HIGH) && ((configuration.sidetone_mode == SIDETONE_ON) || (configuration.sidetone_mode == SIDETONE_PADDLE_ONLY))) {
    return;
  }
...
*/
#if defined(ARDUINO_RASPBERRY_PI_PICO_W) || defined(ARDUINO_RASPBERRY_PI_PICO) || defined(ARDUINO_ARCH_ESP32)
#if defined(DEBUG_EEPROM)
  //debug_serial_port->println(F("write_settings_to_eeprom: ARDUINO_RASPBERRY_PI_PICO"));
  debug_serial_port->println(F("write_settings_to_eeprom: ESP32"));
#endif
  EEPROM.write(0, eeprom_magic_number);
  EEPROM.commit();
  ...
  if ((state) && (key_state == 0)) {
    if (key_tx) {
      ...
      if (current_tx_key_line) {
        ...
        #if tx_led
          digitalWrite(tx_led, tx_key_line_active_state);
        #endif 
      }
    ...
    if ((state == 0) && (key_state)) {
      if (key_tx) {
        ...
        #if tx_led
          digitalWrite(tx_led, tx_key_line_inactive_state);
        #endif 
        }
#if defined(OPTION_WINKEY_2_SUPPORT) && defined(FEATURE_WINKEY_EMULATION) && !defined(FEATURE_SO2R_BASE)
        if ((wk2_both_tx_activated) && (tx_key_line_2)) {
                ...
                if (y < (memory_end(memory_number) + 1)) {
                  ...
                  if ((eeprom_byte_read2 > 48) && (eeprom_byte_read2 < 55)) {
                    switch (eeprom_byte_read2) {
                      ...
                      case 50:
                        if ((ptt_tx_2) || (tx_key_line_2)) { switch_to_tx_silent(2); }
                        break;
                      case 51:
                        if ((ptt_tx_3) || (tx_key_line_3)) { switch_to_tx_silent(3); }
                        break;
                      case 52:
                        if ((ptt_tx_4) || (tx_key_line_4)) { switch_to_tx_silent(4); }
                        break;
                      case 53:
                        if ((ptt_tx_5) || (tx_key_line_5)) { switch_to_tx_silent(5); }
                        break;
                      case 54:
                        if ((ptt_tx_6) || (tx_key_line_6)) { switch_to_tx_silent(6); }
                        break;
                    }
                  }
                }
...
#ifdef FEATURE_ROTARY_ENCODER
#if defined(ARDUINO_ARCH_ESP32)
  encoder.attachSingleEdge(rotary_pin1, rotary_pin2);
  encoder.setCount(configuration.wpm);
#else
...
#ifdef FEATURE_MEMORIES
  enum usb_kbd_states { USB_KEYBOARD_NORMAL,
    ...
    for (int x = 0; ((x < udp_receive_packet_buffer_bytes) && (static_return_value == 0)); x++) {
      ...
      if (!hit_vdu_command) {
        ...
        if ((incoming_byte > 47) && (incoming_byte < 58)) {
          ...
          service_udp_receive();
          if (((udp_receive_packet_buffer_bytes > 0) && ((udp_receive_packet_buffer[0] == 'V') || (udp_receive_packet_buffer[0] == 'D') || (udp_receive_packet_buffer[0] == 'U'))) || (udp_receive_packet_buffer_bytes == 0) || (digits > 4)) {
...
void task_TaskList(void *pvParameter) {
    ...
    while (1) {
        ...
        Serial.println("----------------------------------------------------\r\n");
        Serial.println("task_name   status   priority   stack   num   core\r\n");
        Serial.println((char *)CPU_RunInfo);
        Serial.println("----------------------------------------------------\r\n");
 
        vTaskDelay(2000 / portTICK_PERIOD_MS);
    }
}
//    xTaskCreate(task_TaskList, "TaskList_", 4096, NULL, 1, NULL);


void task_TaskGetRunTimeStats(void *pvParameter) {
    char CPU_RunInfo[400];
    while (1) {
        memset(CPU_RunInfo, 0, 400);
        vTaskGetRunTimeStats((char *)&CPU_RunInfo);
 
        Serial.println("task_name      run_cnt                 usage_rate   \r\n");
        Serial.println((char *)CPU_RunInfo);
        Serial.println("----------------------------------------------------\r\n");
...
//    xTaskCreate(task_TaskGetRunTimeStats, "TaskGetRunTimeStats_", 4096, NULL, 1, NULL);
*/